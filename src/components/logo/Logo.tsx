...
export default function Logo({hideText = false, className = "", col, textStyle="" }: Readonly<{hideText?: boolean; className?: string; col?: boolean, textStyle?: string}>) {
  return (
    <Link to={PATHS.INDEX} className={`flex items-center gap-3 z-50 ${className} ${col && 'max-sm:flex-col max-sm:w-full max-sm:justify-center'}`}>
      ...
      {!hideText && (
        <span className={`text-2xl max-lg:text-lg font-bold tracking-wide max-sm:hidden ${hideText ? "hidden" : ""} max-sm:text-inherit sm:text-white ${textStyle}`}>
          Dschang's Signal
        </span>
      )}
      {col && (
