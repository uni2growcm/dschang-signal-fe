...
export default function Logo({
  ...
  className = '',
  col,
}: Readonly<{ hideText?: boolean; className?: string; col?: boolean }>) {
  return (
    <Link
      to={PATHS.INDEX}
    ...
    >
      ...
      {!hideText && (
        <span
          className={`text-2xl max-lg:text-lg font-bold tracking-wide max-sm:hidden ${hideText ? 'hidden' : ''} max-sm:text-inherit sm:text-white`}
        >
          Dschang's Signal
        </span>
      )}
